"""Tests for suppressions generation: violations -> YAML config."""
...
class TestGenerateSuppressions:
    """Unit tests for generate_suppressions()."""

    def test_empty_violations_returns_empty_rules(self):
        result = generate_suppressions([])
    ...
    def test_single_rule_single_resource(self, make_violation):
        ...
        result = generate_suppressions(violations)
        parsed = yaml.safe_load(result)
        assert parsed == {
            "rules": {
