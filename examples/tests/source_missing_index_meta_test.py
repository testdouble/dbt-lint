...
class TestSourceMissingIndexMeta:
    def test_flags_ft_source_without_meta(self, make_resource, default_config):
        r = make_resource(
            resource_type="source",
            resource_name="ft_something.patients",
            meta={},
        )
        v = source_missing_index_meta(r, default_config)
        assert v is not None
        assert "indexes_reviewed" in v.message

    def test_clean_ft_source_with_meta(self, make_resource, default_config):
        r = make_resource(
            resource_type="source",
            resource_name="ft_something.patients",
            meta={"indexes_reviewed": True},
        )
        assert source_missing_index_meta(r, default_config) is None

    def test_ignores_non_ft_source(self, make_resource, default_config):
        r = make_resource(
            resource_type="source",
